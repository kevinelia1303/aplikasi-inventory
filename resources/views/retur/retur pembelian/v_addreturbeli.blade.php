...
@section('content')
    <h1>Add Retur Pembelian</h1>
    <div class="card card-info">
              <!-- /.card-header -->
              <!-- form start -->
              {{-- action="{{ route('submitDataRetur') }}" --}}
              <form method="post" action="{{ route('submitDataRetur') }}" class="form-horizontal">
                @csrf
                <div class="card-body">
                    <input type="hidden" value="{{ Auth::user()->id }}" name="id_user" required>
                  
                   <div class="form-group row">
                    <label class="col-sm-2 col-form-label">ID Retur Pembelian</label>
                    <div class="col-sm-10">
                      <input type="text" class="form-control" name="id_Transaksi" readonly="" value="{{ 'RR'.date('Y').'-'.date('m').$kd }}" placeholder="ID Retur Beli .." required>
                    </div>
                    </div>
                  {{-- </div>
                  <div class="form-group row">
